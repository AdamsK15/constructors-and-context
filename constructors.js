...
Movie.prototype.changeRating = function (number) {
  let newaverage = this.rating + number
  return newaverage / 2
}
...
function User(name, age, email, savedPosts) {
  ...
  // this.savedPosts = [{ id: Number, title: '', rating: Number }]
}
...
User.prototype.removeSavedPost = function (postId) {
  let deleteIndex = this.savedPosts.findIndex((element) => {
    return element.id === postId

  })
  this.savedPosts.splice(deleteIndex, 1)
...
User.prototype.changePostRating = function (id, newRating) {
  let updatedRating = this.savedPosts.findIndex((element) => {
    return element.id === id

  })
  this.savedPosts[updatedRating].rating = newRating
